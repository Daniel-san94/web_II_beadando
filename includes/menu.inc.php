...
Class Menu {
    ...
    public static function getMenu($sItems) {
        ...
        {
            if($menuitem[1] == "")
            { $menu.= "<li><a href='".SITE_ROOT.$menuindex."' ".($menuindex==$sItems[0]? "class='selected navlink'":"class='navlink'").">".$menuitem[0]."</a></li>"; }
            else if($menuitem[1] == $sItems[0])
            { $submenu .= "<li><a href='".SITE_ROOT.$sItems[0]."/".$menuindex."' ".($menuindex==$sItems[1]? "class='selected navlink'":"class='navlink").">".$menuitem[0]."</a></li>"; }
        }
        $menu.="</ul>";
